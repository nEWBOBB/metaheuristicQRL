...
results_dir = os.path.join(script_dir, 'resultsPLOTS/cartpole/')
...
for folder in os.listdir(results_dir):
	...
	print(folder)
	ax = sns.lineplot(x=bins[:-1], y=grouped_data['mean'], label=folder)
	plt.fill_between(bins[:-1], lower_error, upper_error, alpha=0.2)
	# Extrahiere die Farben der Linien
	line_colors = [line.get_color() for line in ax.lines]

	# Drucke die Farben
	for i, color in enumerate(line_colors):
		print(f"Line {i+1}: {color}")
